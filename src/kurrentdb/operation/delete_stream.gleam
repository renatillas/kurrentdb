...
const delete_method: String = "Delete"
...
pub fn request(
  ...
  let message = encode_request(stream_name, config.expected_revision)

  kurrentdb.grpc_request(
    client,
    "/" <> streams_service <> "/" <> delete_method,
    [message],
  )
}
...
) -> Result(Delete, ResponseError) {
  use messages <- result.try(decode_response_messages(response))

  case messages {
    [] -> Ok(Delete(position: NoPositionReturned))
    [message] ->
      decode_delete_resp(message)
      |> result.map_error(GrpcError)
      |> result.try(map_delete)
...
fn map_delete(response: DeleteResponse) -> Result(Delete, ResponseError) {
  case response {
    DeleteSuccess(position) -> Ok(Delete(position: position))
  }
}

fn decode_delete_resp(
  data: BitArray,
) -> Result(DeleteResponse, kurrentdb.GrpcError) {
  kurrentdb.decode_run(data, operation_position_decoder(DeleteSuccess))
}
